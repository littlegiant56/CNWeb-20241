...
export default function ConversationPanel({
  ...
  const handleCloseMessage = () => {
    setConversations(
      conversations.filter(
        (msg) => msg.receivedUserId !== conversation.receivedUserId
      )
    );
  };
  ...
  return (
    receivedUserProfile && (
      <Container
        className="border p-1 rounded-2 m-0 mb-3 bg-white"
        style={{ width: "300px" }}
      >
        <Container className="d-flex p-0 mt-1 mb-1">
          <img
            src={receivedUserProfile?.avatar}
            alt="avatar"
            style={{
              width: "45px",
              borderRadius: "50%",
          ...
          />
          <h5 className="align-self-center ms-2">
            {receivedUserProfile?.firstName +
              " " +
              receivedUserProfile?.lastName}
          </h5>
          <Button
        ...
        <Container
          id="message-panel"
          className="border rounded-2 p-1 mb-2"
          style={{
            height: "300px",
            overflow: "auto",
            display: "flex",
            ...
            {listMessages.map((msg, index) => {
              return (
                <Container
                  ...
                  style={{ width: "120px" }}
                  className={
                    msg.sentUserId === localStorage.getItem("userId")
                      ? "bg-primary text-white rounded-2 p-1 mb-1 m-0 align-self-end"
                      : "bg-secondary text-white rounded-2 p-1 mb-1 align-self-start m-0"
                  }
                >
                  {msg.content}
                </Container>
