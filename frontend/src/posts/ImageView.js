...
export default function ImageView() {
  ...
  const handleNextImage = () => {
    setImagePosition(
      Math.min(post.image.length - 1, Number(imagePosition) + 1)
    );
  };
  ...
  return (
    post && (
      ...
      >
        <Container
          className="bg-dark d-flex justify-content-between p-0 position-fixed"
          style={{ width: "70%", height: "100%" }}
        >
          <Button
            className="bg-dark text-white border-0 text-decoration-none"
            style={{
              position: "absolute",
              top: "10px",
              width: "10px",
          ...
          <Button
            className="p-0 border-0 align-self-center ms-3"
          ...
          <Image
            ...
            src={post.image[imagePosition]}
            style={{ maxHeight: "100vh", maxWidth: "90%" }}
          />
          <Button
            className="p-0 border-0 align-self-center me-2"
            style={{ width: "30px", background: "transparent" }}
        ...
        >
          <Container className="mb-2">
            ...
            <Container className="p-0">{post.body}</Container>
            {post.dateCreated && (
              <p className="text-secondary">
                {new Date(post.dateCreated.seconds * 1000).toLocaleString()}
              </p>
            )}
