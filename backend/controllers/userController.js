...
export const register = async (req, res, next) => {
  ...
  createUserWithEmailAndPassword(auth, email, password)
    .then((userCredential) => {
      setDoc(doc(db, "profiles", userCredential.user.uid), {}).then(() => {
...
export const updateProfile = async (req, res) => {
  ...
  let avatarCheck = avatarImageFile ? true : false,
    coverCheck = coverImageFile ? true : false;
  Promise.all([
    coverCheck
      ? uploadBytes(cover_storageRef, coverImageFile.buffer, {
          contentType: coverImageFile.mimetype,
        })
      : null,
    avatarCheck
      ? uploadBytes(avatar_storageRef, avatarImageFile.buffer, {
...
export const getAllFriendRequests = async (req, res) => {
  const q = query(
    ...
    where("receivedUserId", "==", req.params.userId)
  );
  let id = [];
